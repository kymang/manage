...
from Amang.utils.dbfunctions import delete_note, get_note, get_note_names, save_note
from Amang.utils.functions import extract_text_and_keyb
...
async def save_notee(_, message):
    if len(message.command) < 2 or not message.reply_to_message:
        ...
        chat_id = message.chat.id if prefix != USERBOT_PREFIX else app2.me.id
...
async def get_notes(_, message):
    ...
    chat_id = app2.me.id if is_ubot else message.chat.id
...
async def get_one_note_userbot(_, message):
    ...
    _note = await get_note(app2.me.id, name)
    if not _note:
