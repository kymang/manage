...
from Amang.utils import formatter
...
from Amang.utils.functions import extract_user, extract_user_and_reason, restart
...
async def ban_globally(_, message):
    ...
    if user_id in [from_user.id, app.me.id] or user_id in SUDOERS:
...
@app.on_message(filters.command("bdu") & SUDOERS)
@capture_err
async def broadcast_message(_, message):
    sleep_time = 0.1
    sent = 0
